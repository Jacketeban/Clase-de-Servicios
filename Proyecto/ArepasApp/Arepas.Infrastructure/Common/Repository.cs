...
namespace Arepas.Infrastructure.Common
{
    public class Repository<T> : IRepository<T> where T : EntityBase
    {
        ...
        public async Task<ResponseData<T>> GetByQueryParamsAsync(QueryParams queryParams)
        {
            ...
            queryParams.Page = (queryParams.Page <= 0) ? PaginationConstants.DefaultPage : queryParams.Page;
            queryParams.Limit = (queryParams.Limit <= 0) ? PaginationConstants.DefaultLimit : queryParams.Limit;

            PaginationData xPagination = new PaginationData(totalCount, queryParams.Page, queryParams.Limit);

            var items = await entityData.Skip((xPagination.Page - 1) * xPagination.Limit)
                                        .Take(xPagination.Limit)
                                        .ToListAsync();
