...
namespace Arepas.Api.Controllers
{
    ...
    public class CustomersController : ControllerBase
    {
        ...
        // GET api/<CustomersController>/5/orders
        [HttpGet("{customerId}/Orders")]
        public async Task<IActionResult> GetOrdersByCustomerId(int customerId)
        {
            return Ok(_mapper.Map<CustomerOrderDto>(await _customerService.GetOrdersByCustomerIdAsync(customerId)));
        }

        // POST: api/v1.0/Customers/Login
        [HttpPost("Login")]
        public async Task<IActionResult> Login([FromBody] LoginDto loginDto)
        {
            await _customerService.LoginAsync(_mapper.Map<Customer>(loginDto));
            return Ok();
        }
    }
}